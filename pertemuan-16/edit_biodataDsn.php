<?php
  ...
  */
  $stmt = mysqli_prepare($conn, "SELECT Bid, kode_dosen, nama_dosen, alamat_rumah, tanggal_jadi_dosen, jja_dosen, homebase_prodi, nomor_hp, nama_pasangan, nama_anak, bidang_ilmu_dosen 
                                    FROM tbl_biodata_dosen WHERE Bid = ? LIMIT 1");
  if (!$stmt) {
    $_SESSION['flash_error'] = 'Query tidak benar.';
    redirect_ke('read_biodataDsn.php');
  }
  ...
  $nama  = $row['kode_dosen'] ?? '';
  $email = $row['nama_dosen'] ?? '';
  $pesan = $row['alamat_rumah'] ?? '';
